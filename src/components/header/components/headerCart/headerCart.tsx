...
const HeaderCart = () => {
  ...
  return (
    <div className={'relative transition'} ref={ref}>
      ...
      {isShowing &&
        <div className={'absolute min-h-100 z-10 top-16 right-0 bg-black text-white rounded-md shadow-lg p-4'}>
          <div className={'flex flex-col min-w-140 w-80 justify-between items-center  mt-6'}>
            <h3 className={'text-lg font-bold absolute top-2 left-2'}>Cart</h3>
            <div>{items.length ? (items.map(item => <CartItem item={item} key={item.id} />)) : (
              <span className={'font-light'}>Cart is empty</span>)}</div>
            <button onClick={() => setIsShowing(false)} className={'absolute top-2 right-2'}>
