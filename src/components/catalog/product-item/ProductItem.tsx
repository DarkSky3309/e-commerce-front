    ...
    <div className={'relative w-fit first:ml-6 animate-scaleIn mb-6'}>
      {isLoaded && <div>
        <div className={'relative'}>
          {user ? <div className={'absolute right-2 top-2 '}>
        ...
        <div>
          ...
          <Link href={`/category/${product.category.slug}`}
                className={'text-xs text-coldBlue'}>{product.category.name}</Link>
          <ProductRating product={product} />
          <span className={'font-bold text-xl'}>${product.price},00 </span>
          <span
            className={'text-gray line-through'}>${Math.floor(product.price + product.price / 100 * 10) + ',00'}</span>
