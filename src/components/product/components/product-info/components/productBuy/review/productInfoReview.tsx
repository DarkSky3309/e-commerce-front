...
const ProductInfoReview: FC<{ id: string | number }> = ({ id }) => {
  const { data } = useQuery(
    ['review', id],
    () => ReviewService.getReviews(id as number),
    { select: ({ data }) => data }
  );
  const [rating, setRating] = useState(
    Math.round(
      data
        ? data?.reduce((acc, reviews) => acc + reviews.rating, 0) / data.length
        : 0
    )
  );
  return data ? (
      ...
      <div className={'flex justify-between border-b-2 border-gray pb-6 mb-6'}>
        <div className={'flex gap-2 items-center'}>
          ...
          <div className={'flex flex-col'}>
            <span className={'font-bold'}>{rating} out of 5 stars</span>
            <Rating
              className={'pb-1'}
              readonly
              initialValue={rating}
        ...
        <div className={'w-1/3'}>
          ...
          <RatingBar
            percent={
              ((data.filter((review) => review.rating === 4).length /
                data.length) *
                100) |
              0
            }
          ...
          <RatingBar
            percent={
              ((data.filter((review) => review.rating === 3).length /
                data.length) *
                100) |
              0
            }
          ...
          <RatingBar
            percent={
              ((data.filter((review) => review.rating === 2).length /
                data.length) *
                100) |
              0
            }
          ...
          <RatingBar
            percent={
              ((data.filter((review) => review.rating === 1).length /
                data.length) *
                100) |
              0
            }
